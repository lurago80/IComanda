# Script para testar a query SQL diretamente no banco de dados
# Identifica erros de colunas inexistentes

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "TESTE DIRETO DA QUERY SQL" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Ler connection string do appsettings.json
...
$connectionString = $appsettings.ConnectionStrings.Firebird

if (-not $connectionString) {
    Write-Host "❌ Connection string não encontrada no appsettings.json!" -ForegroundColor Red
    exit 1
}

Write-Host "📋 Connection String: $($connectionString.Substring(0, [Math]::Min(80, $connectionString.Length)))..." -ForegroundColor Yellow
Write-Host ""

# Verificar se o Firebird está acessível
Write-Host "🔍 Verificando conexão com o banco..." -ForegroundColor Yellow

try {
    # Tentar fazer uma query simples primeiro
    $testQuery = "SELECT COUNT(*) FROM PRODUTOESERVICO"
    Write-Host "   Testando query simples: $testQuery" -ForegroundColor Gray
    
    # Usar isql-fb se disponível, senão usar .NET
    $isqlPath = Get-Command isql-fb -ErrorAction SilentlyContinue
    if ($isqlPath) {
        Write-Host "   Usando isql-fb para teste" -ForegroundColor Gray
    } else {
        Write-Host "   isql-fb não encontrado, tentando via .NET..." -ForegroundColor Gray
    }
} catch {
...
Write-Host ""
Write-Host "📝 Testando a query completa que está falhando..." -ForegroundColor Green
Write-Host ""

# Query que está sendo executada no código
$query = @"
...
$queryFile = Join-Path $env:TEMP "teste_query.sql"
$query | Out-File -FilePath $queryFile -Encoding UTF8

Write-Host "✅ Query salva em: $queryFile" -ForegroundColor Green
Write-Host ""
Write-Host "📋 Para testar manualmente:" -ForegroundColor Yellow
Write-Host "   1. Abra o isql-fb ou Flamerobin" -ForegroundColor White
Write-Host "   2. Conecte ao banco: $($connectionString -replace 'Password=[^;]+', 'Password=***')" -ForegroundColor White
Write-Host "   3. Execute o arquivo: $queryFile" -ForegroundColor White
Write-Host ""
Write-Host "🔍 Verificando colunas que podem estar faltando..." -ForegroundColor Yellow
Write-Host ""

# Lista de colunas que podem estar causando problema
$colunasProblema = @(
    "p4.situacao_rpl",
    "p4.data_alteracao",
    "p2.data_alteracao",
...
foreach ($coluna in $colunasProblema) {
    Write-Host "   Verificando: $coluna" -ForegroundColor Gray
}
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "1. Execute a query manualmente no banco para ver o erro exato" -ForegroundColor White
Write-Host "2. Verifique os logs da API em: logs\icomanda-*.txt" -ForegroundColor White
Write-Host "3. Execute o script TESTAR_PRODUTOS.ps1 para testar via API" -ForegroundColor White
Write-Host ""
